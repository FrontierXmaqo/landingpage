...
/** Colour carries severity, so a problem reads before the number is parsed. */
function Signal({ label, count, sessions, warnAt, badAt }: { label: string; count: number; sessions: number; warnAt: number; badAt: number }) {
  const pct = sessions ? (count / sessions) * 100 : 0;
  const level = pct >= badAt ? "bad" : pct >= warnAt ? "warn" : "good";
  const chip = {
    good: "bg-brand-green-tint text-brand-green-ink",
    warn: "bg-status-warn-bg text-status-warn",
    bad: "bg-status-critical-bg text-status-critical",
  }[level];
  const bar = { good: "bg-brand-green", warn: "bg-brand-orange-deep", bad: "bg-status-critical" }[level];
  const word = { good: "Healthy", warn: "Watch", bad: "Needs attention" }[level];

  return (
    <div className="admin-card flex flex-col gap-2 p-5">
      <div className="flex min-h-9 items-start justify-between gap-2">
        <span className="text-sm font-semibold text-base-ink">{label}</span>
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ${chip}`}>{word}</span>
      </div>
      <p className="text-2xl font-bold tabular-nums text-base-ink">{pct.toFixed(1)}%</p>
      <div className="h-1.5 overflow-hidden rounded-full bg-base-line">
        {/* Scaled ×4 so single-digit percentages are still legible as a bar. */}
        <div className={`h-full rounded-full ${bar}`} style={{ width: `${Math.min(100, pct * 4)}%` }} />
      </div>
      <p className="text-xs tabular-nums text-base-slate">{n0(count)} sessions</p>
    </div>
  );
}
...
export default function Charts({
  ...
  const [showSelangor, setShowSelangor] = useState(false);
  const o = overview;
    ...
    <div className="mt-8 space-y-8">
      ...
      <section>
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h2 className="text-sm font-semibold text-base-ink">Frustration signals</h2>
          <p className="text-xs text-base-slate">Share of sessions showing each behaviour</p>
        </div>
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Signal label="Rage clicks" count={o.rage} sessions={o.sessions} warnAt={3.5} badAt={5.5} />
          <Signal label="Dead clicks" count={o.dead} sessions={o.sessions} warnAt={8} badAt={12} />
          <Signal label="Excessive scrolling" count={o.excess} sessions={o.sessions} warnAt={2} badAt={3} />
          <Signal label="Quick backs" count={o.quick_back} sessions={o.sessions} warnAt={12} badAt={18} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="mt-3 grid grid-cols-1 items-start gap-3 lg:grid-cols-3">
          ...
          <Bars
            title={showSelangor ? "Selangor by district" : "Leads by state"}
            rows={showSelangor ? leads.selangor : leads.byState}
            colour="var(--color-chart-5)"
            empty={showSelangor ? "No Selangor leads with a postcode yet." : undefined}
            action={
              <div className="flex gap-1 rounded-full border border-base-line bg-base-bg p-0.5">
                {[
                  { on: false, label: "States" },
                  { on: true, label: "Selangor" },
                ].map((t) => (
                  <button
                    key={t.label}
                    type="button"
                    onClick={() => setShowSelangor(t.on)}
                    aria-pressed={showSelangor === t.on}
                    className={`rounded-full px-2.5 py-1 text-xs font-semibold transition ${
                      showSelangor === t.on ? "bg-base-panel text-base-ink shadow-sm" : "text-base-slate hover:text-base-ink"
                    }`}
                  >
                    {t.label}
                  </button>
        ...
        {(leads.missingPostcode > 0 || leads.unknownSegment > 0) && (
          <p className="mt-3 text-xs text-base-slate">
            {leads.missingPostcode > 0 && (
              <>
                {leads.missingPostcode} of these leads have no postcode, so they appear under state only — the field was added
                after they came in.{" "}
              </>
            )}
