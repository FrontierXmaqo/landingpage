...
export async function updateFaqItem(id: string, field: string, value: string) {
  const profile = await requireRole([...ROLES]);
...
export async function removeFaqItem(id: string) {
  await requireRole([...ROLES]);
  const supabase = await getSupabaseUserClient();
  await supabase.from("faqs").delete().eq("id", uuid(id)).eq("status", "draft");
  revalidatePath("/admin/faq");
}
