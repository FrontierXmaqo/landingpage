      ...
      {p && (
        <div className="relative">
          ...
          <div className="grid gap-0 md:grid-cols-[5fr_6fr]">
            <Gallery key={p.id} product={p} label={ui.images} />

            {/* Summary */}
            <div className="p-6 sm:p-8">
              <span className="inline-block rounded-md bg-[var(--cat)] px-2.5 py-1 text-xs font-semibold text-white">{ui.categories[p.category]}</span>
              ...
              >
                {ui.quoteWith(p.brand)}
              </Link>
            </div>
          </div>

          <div className="grid gap-8 border-t border-base-line p-6 sm:p-8 md:grid-cols-2">
            <DetailList title={ui.keyFeatures} items={p.features} />
            <DetailList title={ui.benefits} items={p.benefits} />

            <section className="md:col-span-2">
              <h3 className="text-lg font-bold">{ui.specs}</h3>
            ...
            <DetailList title={ui.applications} items={p.applications} />

            <section>
              <h3 className="text-lg font-bold">{ui.faq}</h3>
...
function Gallery({ product, label }: { product: Product; label: string }) {
  ...
  return (
    <div className="bg-white p-6 sm:p-8">
      ...
      {product.images.length > 1 && (
        <div className="mt-4 flex justify-center gap-3" role="group" aria-label={label}>
